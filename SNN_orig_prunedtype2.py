...
def test(model, device, test_loader, epoch, writer=None):
    model.load_state_dict(torch.load('weights_orig.pt'))
    model = prune_model_l1_unstructured(model, nn.Conv2d, 0.4)
    torch.save(model.state_dict(), "weights_orig_prunedtype2.pt")
    model.load_state_dict(torch.load('weights_orig_prunedtype2.pt'))
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
