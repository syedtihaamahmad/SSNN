...
def test(model, device, test_loader, epoch, writer=None):
    ...
    model, {torch.nn.Conv2d,torch.nn.Linear}, dtype=torch.float16 
)
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
#             only_first_spike=False,
        ...
        for epoch in range(epochs):
                ...
                torch.save(model.state_dict(), 'weights_orig_16_PGD'+str(TT[k])+str(v_thh[j])+'.pt')
        ...
        attacks = [fb.attacks.LinfBasicIterativeAttack()]
