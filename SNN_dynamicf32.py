...
def train(model, device, train_loader, optimizer, epoch, writer=None):
    ...
    #torch.save(model.state_dict(), 'weights_orig.pt')

    return losses, mean_loss


def test(model, device, test_loader, epoch, writer=None):
    #model.load_state_dict(torch.load('weights_orig.pt', map_location=torch.device('cpu')))
...
#             only_first_spike=False,
        ...
        for epoch in range(epochs):
            ...
            #torch.save(model.state_dict(), 'weights_orig'+str(TT[k])+str(v_thh[j])+'.pt')
            lthr=0
            uthr=-0
            sd = torch.load('weights_orig'+str(TT[k])+str(v_thh[j])+'.pt')  # load the state dicd
            for kk in sd.keys():
                if not 'weight' in kk:
                ...
                #print(c)
                sd[kk] = w*c   # set to zero weights smaller than thr

                #print(sd[kk])
            torch.save(sd, 'weights_pruned'+str(TT[k])+str(v_thh[j])+'.pt')
            model.load_state_dict(torch.load('weights_orig'+str(TT[k])+str(v_thh[j])+'.pt'))
        ...
        attacks = [fb.attacks.PGD(),fb.attacks.L2FastGradientAttack,fb.attacks.LinfBasicIterativeAttack()]
